...
Backend logic for the Weather Application. Handles API calls, database 
...
def date_calculator(dates):
    """
    Calculates the number of days between start_date and end_date, inclusive. 
    ...
    Returns:
        ...
        total_days: int, the number of days in the span between start and end date, inclusive

    V2 Thoughts:

        This seems like a simple, solid little function that currently does what I need it to do. The only thing that might impact it would be if I change how start_date
        and end_date are calculated/created/handled. If that happens, I'll need to adjust this, but that will only be tertiary to other structural changes. 
    """
...
def read_from_database(latitude, longitude, dates, connection):
    """
    Using sqlite3, runs a search of the database, using lat, lon, and start_date/end_date as primary keys. Selects the date, latitude, longitude, 
    the daily temperature highs, the daily temperature lows, and the weather code. Stores results as a list of tuples. Creates the list db_data and then adds
    the results to the list, creating a list of a list of tuples.

    Args:
        latitude: the latitude coordinate string as generated by geo_convert()
        longitude: the longitude coordinate string as generated by geo_convert()
        dates: a list of dates strings generated by date_converter()
        connection: the connection to the database generated by create_or_connect_database()

    returns:

        db_data: a list containing a list of tuples

    V2 Thoughts:

        I think the major question for all the database functions will be this: are we transforming data to match the API input, or are we transforming the API input to match the data? If the former,
        it might be worth writing a function that does this for any database instance - something like "general_data_convert" and stick it at the end of every database function with results (like this one). 
        However, thinking it over just now, I actually think that it might be smarter to adjust the API results rather than the database data. Why? Because we might change the API in the future. Or there might be
        more APIs we add. Our database is under our control, for the most part. But if I change the API after implementing data transformation on all my database functions, I'll need to go back and update all of those. 
        The point of entry is actually the API, and then for every API (if more were added) we could craft a function to update it to match our data structure, rather than trying to change everything else to match it. 
        In the end, I think that will be the way to go. When data comes in, update it then - don't change everything else to match it. It makes sense how we got here: we started with the API, not the database, so it seemed 
        to make sense to give API the priority. But, that's just a lesson in planning. In fact, if I'd planned this out (as I now would know better how to do) I would have built the database and then would have transformed
        the API data.  
...
def write_to_database(weather, connection, latitude, longitude):
    """
    Using sqlite3, adds the data in the variable weather generated by get_weather_data() to the database.

    Args:

        weather: data returned by get_weather_data().
        connection: the connection to the database generated by create_or_connect_database() 
        latitude: the latitude coordinate string as generated by geo_convert()
        longitude: the longitude coordinate string as generated by geo_convert()
    ...
    V2 Thoughts:
        ...
        See V2 thoughts for read_from_database.
...
def check_database(connection, latitude, longitude, dates):
    """
    Using sqlite3, identifies date ranges not found in the database and returns them as a list for API retrieval
    ...
    Returns:
        Success: (api_call_list, None)
        Error: (None, "Check Database error.")
    
    V2 Thoughts: 
        See V2 thoughts for read_from_database. Also, is there any way to improve the
        way it "checks" for returned records and thus decides to add to the api_call_list? Is
        that a small function on it's own or are the concerns separated enough as-is? 
    ...
    try:
        for start_date, end_date in dates:
            cursor.execute("""
                ...
                """, (latitude, longitude, start_date, end_date))
            results = cursor.fetchall() 
            if not len(results) == total_days: #checks if records returned matches date span
...
def create_or_connect_database():
    """
    Using sqlite3, checks for an existing database, and then creates or connects with the database, "weather.db", 
    ...
    Major thoughts here would center on changing the table structure. Do we need cached_at, for example?
    Any changes to the database would need to be updated across every impacted function. 
    Also, if we do decide on a datastructure, fundamentally that should be based on the final
    database design.
...
def table(weather, new_window):
    """
    Using tkinter and ttk, creates and builds the display table for the UI.
    Creates one table per year.

    Args:
        weather: weather data as returned by get_weather().
        new_window: a tkinter window or frame object  

    Returns:
        None
    
    Thoughts for V2: 
        Is it possible (and valuable) to break this down into smaller functions?
        Part 1 - design the table
        Part 2 - Assign the variables
        Part 3 - Generate the actual table? 
        Could part 3 be a separate function, possibly combined with part 2?
        Eh...but the loop is a fundamental part of creating the table. I'd be storing
        the results of the loop just to unpack them again.
        If the data structure changes, this will need to be updated.   
